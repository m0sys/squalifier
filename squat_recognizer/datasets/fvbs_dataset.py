...
SAMPLE_TO_BALANCE = (
    False  # if true, take at most the mean number of instances per class.
)
...
class FvbsDataset(Dataset):
  ...
  """
    ...
    def load_or_generate_data(self) -> None:
        ...
        self.data = (
            src.transform(get_transforms(), size=self.input_shape[1])
            .databunch()
    ...
    def _process_fvbs_dataset(self) -> None:
        ...
        (PROCESSED_DATA_TRAIN_DIRNAME / "front-squat").mkdir(
            parents=True, exist_ok=True
        )

        (PROCESSED_DATA_VALID_DIRNAME / "back-squat").mkdir(parents=True, exist_ok=True)
        (PROCESSED_DATA_VALID_DIRNAME / "front-squat").mkdir(
            parents=True, exist_ok=True
        )
        ...
        data = (
            src.transform(get_transforms(), size=self.image_size)
            .databunch()
            .normalize(imagenet_stats)
        )
